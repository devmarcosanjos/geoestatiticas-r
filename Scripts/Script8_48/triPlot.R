...
dados <- read.csv("simetrica25_interTri.csv",sep=",",header=TRUE)
...
#pdf("Fig8_35.pdf",width=cx,height=cy)
...
for (i in 1:n)  {
  if (z[i] != code) {
    x1=x[i]-dx/2; y1=y[i]-dy/2
    x2=x[i]+dx/2; y2=y[i]+dy/2
...
eixoy=eixox=c(rep(xmax+dx2,ncores+1))
