...
fronteira <- read.csv("geoLitos_CVX.csv",sep=",",header=TRUE)
...
zp=c(rep(code,nx*ny))  #litologia mais provavel
vp=c(rep(code,nx*ny))  #lito mais provavel com ZI
for (i in 1:ny) {
  for (j in 1:nx) {
  kb=j+(i-1)*nx
...
x=dados$X; y=dados$Y; z=dados$Litos
zmin=min(z)
Jcat=max(z)  #maximo de litologias
n=length(z)
cat("comeca a malha regular")
#interpolacao da malha regular
nppq=1  #pesquisa com 2 pontos por quadrante
npvq=min(4*nppq*4,n)
...
for (i in 1:ny){
  for (j in 1:nx){
    ...
    if (ordem[kb] > 0) {
      ...
      X<-solve(A,C)
      #interpolacao multiquadrica de variavel categorica
      teores=c(rep(0,Jcat))  #no. de tipos
      for (k in 1:Jcat){
        litos=c(rep(0,knt))
        for (l in 1:knt){
          lito=trunc(zpx[l])
        ...
        sz=0
        for (l in 1:knt){sz=sz+X[l]*litos[l]}
        teores[k]=sz
      }
      #encontra o tipo mais provavel
      litoProv=which.max(teores)
      litoVar=litoProv
      sz=max(teores)
      if (sz*(1-sz) > 0.20){litoVar=0}
      kb=j+(i-1)*nx
      zp[kb]=litoProv
      vp[kb]=litoVar
    } 
...
saida=data.frame(xp,yp,zp,vp)
names(saida)[1]=paste("XP")
names(saida)[2]=paste("YP")
names(saida)[3]=paste("Lito")
names(saida)[4]=paste("LitoZI")
write.csv(saida,file="geoLitos_interCat.csv",row.names=FALSE)
cat("fim")
